...
export default function ComprasPage() {
  ...
  // Pontos calculados automaticamente (Total Pago × Pontos por Real)
  const calculatedPoints = totalPaid * pointsPerReal

  // Valor em reais dos pontos (Pontos × Milheiro ÷ 1000)
  const pointsValueInReais = (calculatedPoints * thousand) / 1000

  // Cashback total = cashback extra + valor dos pontos
  const cashbackTotal = (cashback * quantity) + pointsValueInReais

  // Custo final unitário
  const discountPerUnit = (advanceDiscount / 100) * paidValue
  const pointsValuePerUnit = ((calculatedPoints / quantity) * thousand) / 1000
  const finalUnitCost = paidValue + freight - discountPerUnit - cashback - pointsValuePerUnit
  ...
  async function handleSubmit(e) {
    ...
    try {
      // Preparar dados com pontos POR UNIDADE (não total)
      const pointsPerUnit = calculatedPoints / quantity

      const dataToSend = {
        ...formData,
        points: pointsPerUnit.toString() // Pontos por unidade
      }
            ...
            <form onSubmit={handleSubmit}>
              <div className="grid grid-cols-3 gap-4 mb-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Ex: Miri, Nubank, Itaú, Vini..."
                    required
                  />
                </div>
              ...
              <div className="bg-gray-50 rounded p-4 mb-4">
                ...
                <div className="grid grid-cols-5 gap-4 text-sm">
                  ...
                  <div>
                    <p className="text-gray-600">Valor dos Pontos</p>
                    <p className="font-bold text-green-600">R$ {pointsValueInReais.toFixed(2)}</p>
                  </div>
                  <div>
