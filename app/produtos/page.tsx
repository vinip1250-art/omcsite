...
export default function ProdutosPage() {
  ...
  const getProductStatus = (productId) => {
    // Buscar todas as compras deste produto
    const productPurchases = purchases.filter(p => p.productId === productId)
    
    if (productPurchases.length === 0) {
      return { label: 'Sem Compras', variant: 'outline' }
    }
    ...
    // Pegar status da compra mais recente
    const lastPurchase = sortedPurchases[0]

    if (lastPurchase.status === 'SOLD') {
      return { label: 'Vendido', variant: 'destructive' }
    } else if (lastPurchase.status === 'DELIVERED') {
      return { label: 'Em Estoque', variant: 'default' }
    } else {
      return { label: 'A Caminho', variant: 'secondary' }
    }
  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    required
                  />
                  <p className="text-xs text-gray-500 mt-1">Este é o único campo obrigatório</p>
                </div>
                <div>
                  <Label>Modelo</Label>
                  <Input
                    ...
                    placeholder="Ex: 15 Pro (opcional)"
                  />
                </div>
                <div>
