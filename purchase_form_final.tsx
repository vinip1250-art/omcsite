...
export function PurchaseForm({ onSuccess }) {
  ...
  const calculate = () => {
    ...
    // Total pago (quantidade * valor unitário)
    const totalPaid = paidUnit * quantity

    // Desconto antecipado (sobre o total)
    ...
    // Cashback total = dos pontos + extra
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.program) {
      alert('Selecione um Programa/Loja')
      return
    }
    ...
    try {
      // Criar múltiplas compras baseado na quantidade
      const promises = []
        ...
        promises.push(
          fetch('/api/purchases', {
            ...
            body: JSON.stringify({
              ...
              freight: i === 0 ? parseFloat(formData.freight) : 0, // Frete só na primeira
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <a href="/contas" target="_blank" className="text-xs text-blue-600 hover:underline">+ Gerenciar contas</a>
        </div>
        ...
        <div>
          ...
          <a href="/programas" target="_blank" className="text-xs text-blue-600 hover:underline">+ Cadastrar novo</a>
        </div>
