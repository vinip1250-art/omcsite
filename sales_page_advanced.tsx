...
export default function VendasPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Verificar se todos os seriais foram preenchidos
    ...
    try {
      ...
      const promises = selectedPurchases.map(purchaseId => {
        return fetch(`/api/purchases/${purchaseId}`, {
          ...
          body: JSON.stringify({
            status: 'SOLD',
            soldValue: soldValue / selectedPurchases.length, // Dividir valor igualmente
  ...
  // Filtros para vendas realizadas
  const uniqueCustomers = [...new Set(sales.map(s => s.customer).filter(Boolean))].sort()

  const filteredSales = sales.filter(s => {
    if (filterAccount !== 'ALL' && s.account !== filterAccount) return false
